...
class Model:
    def __init__(self):
        self.vgg_r = R19.Model()
        self.vgg_t = T19.Model()
    ...
    def build_model(self):
      ...
      #  rconv3_5 = tf.nn.relu(self._dilated_conv2d(rpool3_5, 3, 128, 3, 0.01, 'rconv3_5', biased = True))
       ...
       ## tpool3_7 = tf.nn.max_pool(tconv3_6, ksize=[1, 7, 7, 1], strides=[1, 1, 1, 1], padding='SAME', name='tpool3_7')
        ...
        attention1 = self.Conv_2d(tf.concat([vgg_r.rconv6_6, vgg_t.tconv6_6], axis=3),
        ...
        self.h_fusion = self.w_r*vgg_r.rconv6_6 + self.w_t*vgg_t.tconv6_6
       ...
       # global1_up2 = self.upsampling_2d(globalf, name='global1_up', size = (8, 8))
        ...
        score_3 = self.Conv_2d(vgg_r.rconv6_6, [3, 3, 128, 2], 0.01, padding='SAME', name = 'score_3' )
        label_3 = self.upsampling_2d(label, name='label_3', size = (0.125, 0.125))
        
        self.score_t= self.Conv_2d(vgg_t.tconv6_6, [3, 3, 128, 2], 0.01, padding='SAME', name = 'score_t' )
        self.pre_t = tf.nn.softmax(self.upsampling_2d(self.score_t, name='pre_3', size = (8, 8)))
       ...
       # conv4 = _conv4*self.global_attention_layer(self.h_fusion, 'conv4')
        ...
        self.Loss_Mean_2 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=score_3,
                                                                                  labels=label_3))
        self.Loss_Mean_4 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.score_t,
                                                                                  labels=label_3))
        ...
        self.Loss_Mean = self.Loss_Mean_1 + self.Loss_Mean_2 + self.Loss_Mean_3 + self.Loss_Mean_4 #+ self.Loss_Mean_5#   self.Loss_Mean_1 + self.Loss_Mean_2 +
